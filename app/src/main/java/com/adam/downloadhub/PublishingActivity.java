...
public class PublishingActivity extends Activity {
    ...
    private View buildUi(){
        ...
        root.addView(Ui.topBar(this,"Publishing Center","العنوان والهاشتاج تلقائيًا قبل النشر",v->finish()));
        ...
        TextView ok=Ui.text(this,"✓  الفيديو جاهز للنشر",19,Ui.TEXT,true);done.addView(ok);
        long mb=video!=null&&video.exists()?video.length()/1024/1024:0;done.addView(Ui.text(this,(video==null?"ملف غير موجود":video.getName())+(mb>0?" • "+mb+" MB":""),11,0xFFC8EBDD,false));root.addView(done);
        ...
        LinearLayout r1=new LinearLayout(this);r1.setOrientation(LinearLayout.HORIZONTAL);r1.addView(platformButton("Instagram","com.instagram.android"),new LinearLayout.LayoutParams(0,Ui.dp(this,52),1f));LinearLayout.LayoutParams t1=new LinearLayout.LayoutParams(0,Ui.dp(this,52),1f);t1.setMargins(Ui.dp(this,8),0,0,0);r1.addView(platformButton("TikTok","com.zhiliaoapp.musically"),t1);root.addView(r1);
        LinearLayout r2=new LinearLayout(this);r2.setOrientation(LinearLayout.HORIZONTAL);LinearLayout.LayoutParams r2p=Ui.matchWrap();r2p.setMargins(0,Ui.dp(this,8),0,0);r2.addView(platformButton("YouTube Shorts","com.google.android.youtube"),new LinearLayout.LayoutParams(0,Ui.dp(this,52),1f));LinearLayout.LayoutParams f1=new LinearLayout.LayoutParams(0,Ui.dp(this,52),1f);f1.setMargins(Ui.dp(this,8),0,0,0);r2.addView(platformButton("Facebook Reels","com.facebook.katana"),f1);root.addView(r2,r2p);
        ...
        Button publish=Ui.accent(this,"نشر / مشاركة الآن");publish.setTextSize(16);publish.setOnClickListener(v->publish());LinearLayout.LayoutParams pub=Ui.matchWrap();pub.height=Ui.dp(this,60);pub.setMargins(0,Ui.dp(this,14),0,0);root.addView(publish,pub);
        TextView note=Ui.text(this,"سيتم فتح شاشة النشر الرسمية للتطبيق المختار مع الفيديو والنص المقترح. يمكنك مراجعة كل شيء قبل الضغط على نشر.",11,Ui.MUTED,false);note.setGravity(Gravity.CENTER);LinearLayout.LayoutParams no=Ui.matchWrap();no.setMargins(0,Ui.dp(this,10),0,0);root.addView(note,no);
        return scroll;
    }

    private Button platformButton(String label,String pkg){Button b=Ui.secondary(this,label);b.setOnClickListener(v->{platform=label;generate();});return b;}
    ...
    private void publish(){
        if(video==null||!video.exists()){toast("ملف الفيديو غير موجود");return;}
        String pkg=packageFor(platform);Uri uri=FileProvider.getUriForFile(this,getPackageName()+".files",video);
        Intent send=new Intent(Intent.ACTION_SEND);send.setType("video/mp4");send.putExtra(Intent.EXTRA_STREAM,uri);send.putExtra(Intent.EXTRA_SUBJECT,text(title));send.putExtra(Intent.EXTRA_TITLE,text(title));send.putExtra(Intent.EXTRA_TEXT,composeText());send.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);send.setClipData(android.content.ClipData.newRawUri("reel",uri));
        if(pkg!=null)send.setPackage(pkg);
        try{startActivity(send);}catch(ActivityNotFoundException e){send.setPackage(null);try{startActivity(Intent.createChooser(send,"نشر الريل"));}catch(Exception ex){toast("لا يوجد تطبيق متاح للنشر");}}
    }

    private String packageFor(String p){if(p.startsWith("Instagram"))return"com.instagram.android";if(p.startsWith("TikTok"))return"com.zhiliaoapp.musically";if(p.startsWith("YouTube"))return"com.google.android.youtube";if(p.startsWith("Facebook"))return"com.facebook.katana";return null;}
