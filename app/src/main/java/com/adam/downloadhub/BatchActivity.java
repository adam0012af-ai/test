...
public class BatchActivity extends Activity {
    ...
    private View buildUi(){
        ...
        TextView sub=Ui.text(this,"ضع رابطًا في كل سطر. سيتم فحص كل رابط واختيار أفضل فيديو صالح تلقائيًا، أو الصوت إذا كان وضعك الافتراضي صوت فقط.",13,Ui.MUTED,false);sub.setGravity(Gravity.CENTER);sub.setLineSpacing(0,1.2f);LinearLayout.LayoutParams sp=Ui.matchWrap();sp.setMargins(0,Ui.dp(this,7),0,Ui.dp(this,16));root.addView(sub,sp);
        ...
        Button start=Ui.primary(this,"تحليل وإضافة أفضل الخيارات");start.setOnClickListener(v->startBatch());LinearLayout.LayoutParams bp=Ui.matchWrap();bp.height=Ui.dp(this,54);bp.setMargins(0,Ui.dp(this,12),0,0);card.addView(start,bp);root.addView(card);
    ...
    private void startBatch(){
        ...
        String raw=input.getText()==null?"":input.getText().toString();String[] lines=raw.split("\\r?\\n");List<String> urls=new ArrayList<>();for(String s:lines){s=s.trim();if(s.startsWith("http://")||s.startsWith("https://"))urls.add(s);if(urls.size()>=100)break;}
        ...
        executor.execute(()->{int ok=0,fail=0;for(int i=0;i<urls.size();i++){String u=urls.get(i);final int pos=i+1;runOnUiThread(()->status.setText("تحليل "+pos+" / "+urls.size()+"…"));try{PlatformExtractor.MediaBundle b=PlatformExtractor.extractOptions(u);MediaOption pick=pick(b.options);if(pick==null)throw new IllegalStateException("لا يوجد خيار مناسب");DownloadEngine.enqueue(this,pick);HistoryStore.add(this,u,"Batch • "+b.platform);ok++;}catch(Exception e){fail++;}}int a=ok,f=fail;running=false;runOnUiThread(()->status.setText("اكتمل Batch ✅\nتمت الإضافة: "+a+"\nتعذر: "+f+"\nراجع مدير التحميلات لمتابعة التقدم."));});
    }
